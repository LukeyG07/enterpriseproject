// server.js (updated for root static without subdirectories)
...
// Fallback: serve index.html on GET /
app.get('/', (req, res) => {
...
[ d.name,d.category_id,d.price,d.description,img,d.socket,d.ram_type,d.memory_size,d.chipset,d.form_factor,d.capacity,d.wattage,d.efficiency,d.case_size,d.fan_size,d.cooler_type ]
);
const pid = r.rows[0].id;
await client.query('INSERT INTO inventory(product_id,stock) VALUES($1,$2)', [pid, d.stock || 0]);
...
[d.name,d.category_id,d.price,d.description,d.socket,d.ram_type,d.memory_size,d.chipset,d.form_factor,d.capacity,d.wattage,d.efficiency,d.case_size,d.fan_size,d.cooler_type,id]
);
if (req.file) { const u = /uploads/${req.file.filename}; await client.query('UPDATE products SET image_url=$1 WHERE id=$2',[u,id]); }
res.json({ success: true });
} catch (e) { res.status(500).json({ error: e.message }); }
});
app.delete('/api/admin/products/:id', requireAdmin, async (req, res) => { await client.query('DELETE FROM products WHERE id=$1',[req.params.id]); res.json({ success: true }); });
app.put('/api/admin/inventory/:id', requireAdmin, async (req, res) => { await client.query('UPDATE inventory SET stock=$1 WHERE product_id=$2',[req.body.stock,req.params.id]); res.json({ success: true }); });
